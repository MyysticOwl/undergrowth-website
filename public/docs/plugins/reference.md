...
*Expose workflows as specialized tools to external AI agents.*

### `mcp:tool_entry`
**Icon:** 🔧 | **Category:** `Integrations/MCP`
**Role:** `Source`
Defines a workflow as an exposed MCP Tool.
- **Config**:
...
- **Behavior**:
  ...
  - When the tool is called by an MCP Client (e.g., Claude Desktop), this component emits the provided arguments.

### `mcp:execute_tool`
**Icon:** 🌉 | **Category:** `Integrations/MCP`
**Role:** `Process`
Execute tools dynamically using MCP.
- Acts as a bridge to execute tools.
- **Inputs**: Expects a JSON object with tool arguments.
